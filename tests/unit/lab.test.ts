...
describe('a recipe written as seven tasks, made into one', () => {
  /** The shape this board actually has: a goal, a recipe, then the culture. */
  const attempt = () => {
    const h = harness('2026-08-10T08:00');
    const project = h.app.addProject('Fibrous composites').id;
    const milestone = h.app.addNode(project, 'Annulus fibrosis').id;
    const goal = h.app.addNode(milestone, 'Chitogel attempt').id;
    const tasks = ['Fabricate ELAC thread', 'Briefly soak in chitogel', 'Wrap around needle', 'Crosslink']
      .map((name) => h.app.addNode(goal, name).id);
    return { h, goal, tasks };
  };

  it('leaves one task where there were four, with the recipe as its steps', () => {
    const { h, goal, tasks } = attempt();
    const made = h.app.combineTasks(tasks, 'Prepare scaffolds');

    const kids = h.app.tree()[0]!.children[0]!.children[0]!.children;
    expect(kids.map((k) => k.name)).toEqual(['Prepare scaffolds']);
    expect(h.app.node(made.id).steps.map((s) => s.text)).toEqual([
      'Fabricate ELAC thread',
      'Briefly soak in chitogel',
      'Wrap around needle',
      'Crosslink',
    ]);
    // Where the run was, not appended after everything else.
    expect(h.app.node(made.id).seq).toBe(1);
    void goal;
  });

  it('keeps what was already ticked off ticked', () => {
    const { h, tasks } = attempt();
    h.app.complete(tasks[0]!);
    h.app.complete(tasks[1]!);

    const made = h.app.combineTasks(tasks);
    expect(h.app.node(made.id).steps.map((s) => s.done)).toEqual([true, true, false, false]);
    // The task itself is not finished: there is work left in it.
    expect(h.app.node(made.id).status).not.toBe('done');
  });
  ...
  it('is one undo step, however many it swallowed', () => {
    const { h, tasks } = attempt();
    const before = h.app.tree()[0]!.children[0]!.children[0]!.children.map((k) => k.name);

    h.app.combineTasks(tasks);
    h.app.undo();

    expect(h.app.tree()[0]!.children[0]!.children[0]!.children.map((k) => k.name)).toEqual(before);
  });
  ...
  it('keeps the notes that were on them, under the names they belonged to', () => {
    const { h, tasks } = attempt();
    h.app.updateNode(tasks[1]!, { notes: 'Chitogel at 4C, thirty seconds' });
    const made = h.app.combineTasks(tasks);
    expect(h.app.node(made.id).notes).toContain('Briefly soak in chitogel: Chitogel at 4C');
  });

  it('leaves the pool with one row instead of a queue', () => {
    const { h, tasks } = attempt();
    // Sequential by default, so the pool showed the first of four; now it
    // shows the one thing there is to pick up.
    expect(h.app.ready().filter((r) => tasks.includes(r.id))).toHaveLength(1);
    const made = h.app.combineTasks(tasks);
    const pool = h.app.ready();
    expect(pool.filter((r) => r.id === made.id)).toHaveLength(1);
    expect(pool.find((r) => r.id === made.id)?.stepsTotal).toBe(4);
  });
});
